...
type EventStore interface {
	...
	// aggregateID: 聚合根 ID
	GetEvents(ctx context.Context, aggregateID string) ([]*kernel.EventRecord, error)

	// GetEventsByType 按类型获取事件（用于分析和审计）
	// eventType: 事件类型
	// limit: 限制数量
	GetEventsByType(ctx context.Context, eventType string, limit int) ([]*kernel.EventRecord, error)
}
