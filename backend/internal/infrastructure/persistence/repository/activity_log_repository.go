...
func NewActivityLogRepository(db *dao.Query) aggregate.ActivityLogRepository {
	return &ActivityLogRepositoryImpl{query: db}
}

// Save 保存活动日志
func (r *ActivityLogRepositoryImpl) Save(ctx context.Context, log *aggregate.ActivityLog) error {
	daoModel := r.fromDomain(log)
	return r.query.ActivityLog.WithContext(ctx).Create(daoModel)
}

// FindByUserID 按用户 ID 查询
func (r *ActivityLogRepositoryImpl) FindByUserID(ctx context.Context, userID int64, limit int) ([]*aggregate.ActivityLog, error) {
	...
	logs := make([]*aggregate.ActivityLog, len(daoModels))
...
func (r *ActivityLogRepositoryImpl) FindByAction(ctx context.Context, action aggregate.ActivityType, limit int) ([]*aggregate.ActivityLog, error) {
	daoModels, err := r.query.ActivityLog.WithContext(ctx).
...
// FindFailed 查询失败的活动
func (r *ActivityLogRepositoryImpl) FindFailed(ctx context.Context, limit int) ([]*aggregate.ActivityLog, error) {
	daoModels, err := r.query.ActivityLog.WithContext(ctx).
		Where(r.query.ActivityLog.Status.Eq(int16(aggregate.ActivityStatusFailed))).
		Order(r.query.ActivityLog.OccurredAt.Desc()).
		Limit(limit).
		Find()
	if err != nil {
...
func (r *ActivityLogRepositoryImpl) fromDomain(log *aggregate.ActivityLog) *model.ActivityLog {
...
func (r *ActivityLogRepositoryImpl) toDomain(m *model.ActivityLog) *aggregate.ActivityLog {
	...
	var status aggregate.ActivityStatus
	if m.Status != nil {
		status = aggregate.ActivityStatus(*m.Status)
	}

	return &aggregate.ActivityLog{
		ID:         m.ID,
		TenantID:   tenantID,
		UserID:     m.UserID,
		Action:     aggregate.ActivityType(m.Action),
		Status:     status,
